...
epsilon = 0.01;  % for our epsilon greedy policy, suggested: 0.01
nStates = 20; 
nActions = 2; 
MAX_N_EPISODES=1e5; %suggested: 1e5 with epsilon = 0.01 and alpha = 3*1e-1 for precision of 0.01.
...
for ei=1:MAX_N_EPISODES,
  ...
  while( 1 ) 
    ...
    if( ~( stp1==1) ) % stp1 is not the terminal state
      Q(st,at) = Q(st,at) + alpha*( rew + gamma*Q(stp1,atp1) - Q(st,at) );
